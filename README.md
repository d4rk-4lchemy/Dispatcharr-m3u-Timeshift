...
Positive numeric `timeshift` values mean archive days. `0`, negative, missing,
and non-numeric values are treated as no catch-up.
...
- `pace_hls_archive`: throttle bridged HLS archive output to `#EXTINF` timing,
  default `true`. Output is paced in smaller chunks within each segment so the
  client keeps receiving data while Dispatcharr's Active Connections stays
  aligned with playback instead of ending the session as soon as the buffered
  programme was downloaded.
...
When a positive `timeshift` value is found, the scanner writes Dispatcharr's
native M3U archive fields (`tv_archive=1` and `tv_archive_duration=<days>`) and
marks them as plugin-managed. If a later scan no longer sees `timeshift`, only
plugin-managed archive fields are removed. Provider-supplied native archive
